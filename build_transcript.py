...
# Config
DEEPGRAM_API_KEY = "f98955fa55e3569e14be4b18b660e464f820f543"
...
OUTPUT_FILE = "transcript.txt"  # Fixed filename, will be overwritten each run
                ...
                if alternatives:
                    ...
                    if is_final and transcript.strip():
                        ...
                        with open(output_file, 'a') as f:
                            ...
                            f.flush()
                        print(
                            f"[FINAL] {timestamp} {transcript} (conf={confidence:.2f})", flush=True)
...
async def main():
    ...
    async with websockets.connect(DG_URL, additional_headers=HEADERS, max_size=None) as websocket:
        ...
        recv_task = asyncio.create_task(
            receive_messages(websocket, OUTPUT_FILE))
        ...
        if send_task in done:
            ...
            if send_task.exception():
                print(
                    f"DEBUG: send_task exception: {send_task.exception()}", flush=True)
        if recv_task in done:
            print("DEBUG: recv_task completed", flush=True)
            if recv_task.exception():
                print(
                    f"DEBUG: recv_task exception: {recv_task.exception()}", flush=True)
